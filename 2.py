...
class TestYOLOReportGeneration(unittest.TestCase):
    ...
    def test_report_generation(self):
        ...
        dog_detected = False  # Переменная для отслеживания обнаружения класса "dog"
            ...
            for img_file, result in results:
                ...
                if boxes is not None and len(boxes) > 0:
                    for box in boxes.data:
                        ...
                        # Проверка на наличие класса "dog"
                        if class_name == 'dog':
                            dog_detected = True
                else:
                    report_file.write('Объекты не обнаружены.\n')

            # Используем assert для проверки наличия класса "dog"
            self.assertTrue(dog_detected, "Ожидаемый объект класса 'dog' не был обнаружен.")

            # Запись результата теста в отчет
            report_file.write('Тест пройден: объект класса "dog" был обнаружен.\n')
